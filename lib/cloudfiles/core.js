...
Cloudfiles.prototype.addFiles = function (container, options, callback){
  var self = this;
  var batch = options.map(function (option) {
    var exists = fs.existsSync(option.local);
    
    // If file exists - get it
    // If not report about error
    return exists ?
        function (callback) {
          self.addFile(container, option, callback);
        } :
        function (callback) {
          callback(Error('File : ' + option.local + ' doesn\'t exist'));
        };
  });
